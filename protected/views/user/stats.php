  ...
  for ($i = 0; $i <= 9; $i++) {
    ...
    foreach ($psg_model as $prdemt_semestr) {
      if ($prdemt_semestr->semestr_id == $do_increment) {
        if (isset($prdemt_semestr->predmet)) {
          ...
          echo CHtml::dropDownList("resultts[$do_increment][$prdemt_semestr->predmet_id]", $avdug, CHtml::listData($rating, 'id', 'value'), $disabled);
        }
      }
    }
    echo "</ul>";
    ...
    foreach ($psg_model as $prdemt_semestr) {
      if ($prdemt_semestr->semestr_id == $posle_increment) {
        ...
        echo CHtml::dropDownList("resultts[$posle_increment][$prdemt_semestr->predmet_id]", $avdrug, CHtml::listData($rating, 'id', 'value'), $disabled);
